...
#     net_output = f"({int(path._net_gates[0].get_gate().id)},"\
#                  f"{int(path._net_gates[1].get_gate().id)})"
...
if len(argv) != 5:
    print("Usage: python3 filename main [chip nr] [netist file nr]",
          "[nr of restarts] [nr of reverts before restart]")
    exit(2)
...
best_df = pd.DataFrame({"netlist": [(hill_list)], "score": [costs[2]],
                        "intersections": [costs[1]], "restarts": [argv[3]],
                        "max_reverts": [argv[4]]})
...
plot_graph(board._nodes, board._paths,
           board._dimensions[0], board._dimensions[1])
